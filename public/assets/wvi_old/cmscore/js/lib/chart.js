		...
		Chart.Tooltip = Chart.Element.extend({
			...
			update: function(changed) {
				...
				if (active.length) {
					...
					helpers.each(tooltipItems, function(tooltipItem) {
						pushOrConcat(labelColors, opts.callbacks.labelColor.call(me, tooltipItem, me._chart));
					});
			...
			drawBody: function(pt, vm, ctx, opacity) {
				...
				helpers.each(body, function(bodyItem, i) {
					...
					helpers.each(bodyItem.lines, function(line, index) {
						...
						if (drawColorBoxes) {
							...
							// Border
							var borderColor = vm.labelColors[i].borderColor;
							if (helpers.isArray(vm.labelColors[i])) borderColor = vm.labelColors[i][index].borderColor;

							ctx.strokeStyle = mergeOpacity(borderColor, opacity);
							ctx.strokeRect(pt.x, pt.y, bodyFontSize, bodyFontSize);


							// Inner square
							var backgroundColor = vm.labelColors[i].backgroundColor;
							if (helpers.isArray(vm.labelColors[i])) backgroundColor = vm.labelColors[i][index].backgroundColor;

							ctx.fillStyle = mergeOpacity(backgroundColor, opacity);
